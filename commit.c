...
int head_update(const ObjectID *new_commit) {
    ...
    /* Make sure parent directories exist (e.g. .pes/refs/heads/) */
    char dir_copy[520];
    ...
    if (slash) {
        *slash = '\0';
        ...
        char tmp[520];
        snprintf(tmp, sizeof(tmp), "mkdir -p %s", dir_copy);
        system(tmp);
    }
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    /* 3. Read parent (may not exist for first commit) */
    ObjectID parent_id;
